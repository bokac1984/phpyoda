$(document).ready(function() {
  ...
  $(document).on('click', '.mark-visible', function(e) {
    e.preventDefault();
    var ids = ph.getSelectedIds();
    
    if (ids.length == 0) {
      return;
    }
    
    var show = ($(this).attr('data-set')).indexOf('show') === -1 ? false : true;
    $.ajax({
      type: 'POST',
      url: '/photo/pictures/showHidePictures',
      data: {id: ids, show: show},
      dataType: 'json',
      success: function(data) {
        if (data.success) {
          if (!show) {
            $('.selected-image').removeClass('displayed-img').addClass('hidden-img');
          } else {
            $('.selected-image').removeClass('hidden-img').addClass('displayed-img');
          }
          $('#disselect-all').click();
        } else {
          console.log('Error happened on setting hidden/shown');
        }
      }
    });
  });
