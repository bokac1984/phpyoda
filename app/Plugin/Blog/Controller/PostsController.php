...
class PostsController extends BlogAppController {
  ...
  public function beforeFilter() {
    parent::beforeFilter();
    $this->Auth->allow(array('index', 'view', 'latest', 'popular', 'search'));
  ...
  public function search($what, $term) {
    $model = Inflector::camelize($what);
    debug(isset($this->Post->hasAndBelongsToMany[$model]) || isset($this->Post->belongsTo[$model]));
    $conditions = array(
        'conditions' => array(
            "$model" => array(
                'conditions' => array(
    ...
    debug($this->Post->find('all', $conditions));
    debug($this->Post->getLastQuery());
  }
