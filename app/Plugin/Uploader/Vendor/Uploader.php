...
class Uploader {
	...
	/**
	 * Returns a file extension for a given mimetype.
	 *
	 * @access public
	 ...
	 * @static
	 */
	public static function extFromMime($mimeType) {
		if (empty(self::$_exts)) {
			if (empty(self::$_mimeTypes)) {
				self::$_mimeTypes = Configure::read('Uploader.mimeTypes');
			}

			foreach (self::$_mimeTypes as $group => $exts) {
				foreach ($exts as $ext => $mimes) {
		...
		if (isset(self::$_exts[$mimeType])) {
			return self::$_exts[$mimeType];
		}

		// Fallback by returning the part following the slash
	...
	public function importRemote($url, array $options = array()) {
		...
		// Remove any trailing query string from the URL basename
		if ($queryLoc = strpos($this->_current, '?')) {
			$this->_current = substr($this->_current, 0, $queryLoc);
		}
		...
		$this->_data[$this->_current]['type'] = curl_getinfo($curl, CURLINFO_CONTENT_TYPE);

		// Strip any encoding off the end of a mimetype like "text/html; charset=ISO-8859-1"
		if ($semicolonPos = strpos($this->_data[$this->_current]['type'], ';')) {
			$this->_data[$this->_current]['type'] = substr($this->_data[$this->_current]['type'], 0, $semicolonPos);
		}

		$this->_data[$this->_current]['ext'] = self::ext($this->_current);

		// The URL had no file extension, fallback to using one generated by the mimetype
		if (empty($this->_data[$this->_current]['ext'])) {
			$this->_data[$this->_current]['ext'] = self::extFromMime($this->_data[$this->_current]['type']);
		}
		...
		if (file_put_contents($dest, $file)) {
			$this->_data[$this->_current]['uploaded'] = date('Y-m-d H:i:s');
